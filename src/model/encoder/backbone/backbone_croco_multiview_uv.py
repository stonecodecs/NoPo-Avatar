...
"""
Backbone that uses croco multiview encoder with self-attention to UV template + input images,
cross-attention between UV template and input images as before.

Key differences:
- Omits out background tokens in fully masked regions.
- Localizes the UV template to different regions (head, torso, legs, hands, etc.)
  - uses custom UV SMPL-X based positional encodings (try coarse, then coarse2fine)
- Grouped local part attention between collection of input tokens with only the face template
- Global downsampled token attention between all input tokens 
"""
...
class AsymmetricCroCoMultiUV(CroCoNet):
    ...
    def __init__(self, cfg: BackboneCrocoMultiUVCfg, d_in: int) -> None:
